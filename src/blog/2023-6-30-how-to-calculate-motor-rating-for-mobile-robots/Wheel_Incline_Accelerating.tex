...
\tikzstyle{torque}=[mygreen,thick,line cap=round]

% FORCES SWITCH
\tikzstyle{force}=[->,myred,thick,line cap=round]
\tikzstyle{Fproj}=[force,myred!40]
\tikzstyle{accl}=[force,mygreen!40]
\newcommand{\vbF}{\vb{F}}
...
\begin{tikzpicture}
  ...
  \ifthenelse{\boolean{showforces}}{
    ...
    \draw[->, torque, rotate=\ang] (\mx+0.3*\r,2.3*\r) arc[radius=\r*1.0, start angle=90, end angle=0] node[right=2, above=2] {$\vb\tau$};

    \draw[accl] (\ang:\mx+1.2*\r)++(\ang+90:1.0*\r) --++ (\ang:{\F*sin(\ang)}) node[above=-2] {$\vb\alpha$};
